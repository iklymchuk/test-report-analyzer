...
def ingest_test_report(
    file_path: str, project: str = None, branch: str = None, commit_sha: str = None
) -> bool:
    """
    ...
    print(
        f"  Passed: {test_run['passed']} ({test_run['passed']/test_run['total_tests']*100:.1f}%)"
    )
    ...
    # Show failed tests
    failed_tests = [
        tc for tc in data["test_cases"] if tc["status"] in ["failed", "error"]
    ]
    if failed_tests:
    ...
    slow_tests = [
        tc for tc in data["test_cases"] if tc.get("duration_seconds", 0) > 5.0
    ]
    if slow_tests:
        print(f"\n🐌 Slow Tests (>5s) ({len(slow_tests)}):")
        for test in sorted(
            slow_tests, key=lambda x: x["duration_seconds"], reverse=True
        )[:5]:
            full_name = f"{test.get('classname', '')}::{test['name']}"
...
Examples:
  ...
  python ingest_report.py test-results.xml \\
    ...
    parser.add_argument("file", help="Path to JUnit XML test report file")
    parser.add_argument("--project", help='Project name (e.g., "my-app")', default=None)
    parser.add_argument(
        "--branch", help='Git branch name (e.g., "main", "develop")', default=None
    )
    parser.add_argument("--commit", help="Git commit SHA", default=None)
    parser.add_argument(
        "--init-db", action="store_true", help="Initialize database before ingestion"
    )

...
def demo_flaky_tests(db, project: str):
    ...
    for i, test in enumerate(flaky_tests[:5], 1):
        print(f"\n{i}. {test['test']}")
        print(f"   Flakiness: {test['flakiness_score']*100:.1f}% ({test['failures']}/{test['total_runs']} failures)")
...
def demo_slow_tests(db, project: str):
    ...
    for i, test in enumerate(slow_tests[:5], 1):
        print(f"\n{i}. {test['test']}")
        print(f"   Avg: {test['avg_duration']:.2f}s | Max: {test['max_duration']:.2f}s | Min: {test['min_duration']:.2f}s")
        if 'p50' in test:
            print(f"   Percentiles - P50: {test['p50']:.2f}s | P95: {test['p95']:.2f}s | P99: {test['p99']:.2f}s")
        print(f"   Runs: {test['run_count']}")
    
    # Performance regressions
    print("\n--- Performance Regressions ---")
    regressions = detect_performance_regressions(db, project, lookback_days=7, comparison_days=30)
    
    if regressions:
        for i, reg in enumerate(regressions[:5], 1):
            print(f"\n{i}. {reg['test']}")
            print(f"   Baseline: {reg['baseline_avg']:.2f}s → Recent: {reg['recent_avg']:.2f}s")
            print(f"   Increase: +{reg['increase_seconds']:.2f}s (+{reg['increase_percent']:.1f}%)")
    else:
    ...
    if outliers:
        for i, outlier in enumerate(outliers[:3], 1):
            ...
            print(f"   Coefficient of Variation: {outlier['coefficient_of_variation']:.1f}%")
    else:
...
def demo_failure_clustering(db, project: str):
    ...
    for i, cluster in enumerate(clusters[:5], 1):
        print(f"\n{i}. {cluster['pattern']}")
        print(f"   Count: {cluster['count']} failures across {cluster['unique_tests']} unique tests")
    ...
    if spikes:
        for i, spike in enumerate(spikes[:5], 1):
            ...
            print(f"   Runs: {spike['test_runs']} | Tests: {spike['total_tests']} | Failures: {spike['total_failures']}")
    else:
...
def demo_trends(db, project: str):
    ...
    if growth['has_data']:
        ...
        print(f"Change: {growth['change']:+.0f} tests ({growth['change_percent']:+.1f}%)")
    ...
    if failure_trend['has_data']:
        ...
        print(f"Range: {failure_trend['min_rate']:.2f}% - {failure_trend['max_rate']:.2f}%")
    ...
    if duration_trend['has_data']:
        ...
        print(f"Change: {duration_trend['change']:+.2f}s ({duration_trend['change_percent']:+.1f}%)")
    ...
    if anomalies:
        for i, anomaly in enumerate(anomalies[:5], 1):
            ...
            for reason in anomaly['anomalies']:
                print(f"   {reason['metric']}: {reason['value']} (avg: {reason['avg']}, z-score: {reason['z_score']})")
    else:
    ...
    if daily:
        print(f"{'Date':<12} {'Runs':<6} {'Tests':<7} {'Pass%':<7} {'Fail%':<7} {'Duration':<10}")
        print("-" * 60)
        for day in daily:
            print(f"{day['date']:<12} {day['runs']:<6} {day['total_tests']:<7} "

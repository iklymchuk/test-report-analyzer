...
def detect_flaky_tests(
    ...
    test_cases = (
        db.query(TestCase)
        .filter(TestCase.test_run_id.in_(run_ids))
        .order_by(TestCase.test_run_id)
        .all()
    )
    ...
    for test_key, results in test_results.items():
        ...
        avg_duration = (
            sum(r["duration"] for r in results if r["duration"])
            / len([r for r in results if r["duration"]])
            if any(r["duration"] for r in results)
            else 0
        )
...
def get_flaky_test_details(
    ...
    query = (
        ...
        .join(TestRun)
        .filter(TestCase.test_run_id.in_(run_ids), TestCase.name == name)
    )
    ...
    errors = [
        case.error_message for case, _ in results if case.status in ["failed", "error"]
    ]
    ...
    first_failure = next(
        (ts for s, ts in zip(statuses, timestamps) if s in ["failed", "error"]), None
    )
    last_success = next(
        (
            ts
            for s, ts in zip(reversed(statuses), reversed(timestamps))
            if s == "passed"
        ),
        None,
    )
    ...
    return {
        ...
        "avg_duration": (
            round(sum(d for d in durations if d) / len([d for d in durations if d]), 3)
            if any(durations)
            else 0
        ),

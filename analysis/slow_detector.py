    ...
    for row in results:
        ...
        if include_percentiles:
            percentiles = _calculate_percentiles(
                db, project, row.classname, row.name
            )
...
def detect_performance_regressions(
    ...
    test_cases = db.query(TestCase, TestRun).join(TestRun).filter(
        TestRun.project == project,
...
def get_test_duration_history(
    ...
    return {
        ...
        'duration_history': [round(d, 3) for d in reversed(durations)],  # Chronological order
...
def get_duration_outliers(
    ...
    for row in results:
        ...
        durations_query = db.query(TestCase.duration_seconds).join(TestRun).filter(
            TestRun.project == project,
...
def _calculate_percentiles(
    ...
    # Get all durations
    query = db.query(TestCase.duration_seconds).join(TestRun).filter(
        TestRun.project == project,
        TestCase.name == name,
        TestCase.duration_seconds.isnot(None)
    )
...
def get_slow_test_summary(
    ...
    Returns:
        Summary statistics
    """
    slow_tests = detect_slow_tests(db, project, threshold_seconds, include_percentiles=False)

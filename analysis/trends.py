...
def compare_time_periods(
    ...
    period1_runs = db.query(TestRun).filter(
        TestRun.project == project,
        TestRun.timestamp >= period1_start,
        TestRun.timestamp <= period1_end
    ).all()
    
    period2_runs = db.query(TestRun).filter(
        TestRun.project == project,
    ...
    # Calculate changes
    failure_rate_change = period1_metrics['failure_rate'] - period2_metrics['failure_rate']

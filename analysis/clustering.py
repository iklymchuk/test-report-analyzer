...
def cluster_failures(
    ...
    failures = db.query(TestCase, TestRun).join(TestRun).filter(
    ...
    for case, run in failures:
        ...
        clusters[pattern].append({
            ...
            'error_message': case.error_message[:500] if case.error_message else '',  # Truncate
    ...
    for pattern, failure_list in clusters.items():
        ...
        result.append({
            ...
            'occurrences': sorted(failure_list, key=lambda x: x['timestamp'], reverse=True)[:5]
        })
    
    return sorted(result, key=lambda x: x['count'], reverse=True)


def cluster_by_module(
    db: Session,
    project: str,
    lookback_days: int = 7
) -> List[Dict]:
...
def cluster_by_time(
    ...
    runs = db.query(TestRun).filter(
        TestRun.project == project,
        TestRun.timestamp >= cutoff
    ).order_by(TestRun.timestamp).all()
    ...
    for window_start, window_runs in windows.items():
        ...
        result.append({
            ...
            'avg_duration': round(sum(r.duration_seconds for r in window_runs if r.duration_seconds) / len(window_runs), 2)
        })
...
def find_related_failures(
    ...
    all_failures = db.query(TestCase, TestRun).join(TestRun).filter(
        TestRun.project == project,
        TestRun.timestamp >= cutoff,
        TestCase.status.in_(['failed', 'error'])
    ).all()
    ...
    for case, run in all_failures:
        ...
        if error_pattern in target_patterns:
            ...
            related.append({
                ...
                'error_type': case.error_type,
                'sample_error': case.error_message[:200] if case.error_message else '',
                'timestamp': run.timestamp.isoformat()
            })
...
def _extract_error_pattern(error_message: Optional[str], error_type: Optional[str]) -> str:
    """
...
def get_failure_summary(db: Session, project: str, lookback_days: int = 7) -> Dict:
    ...
    # Get all failures
    failures = db.query(TestCase).join(TestRun).filter(
        TestRun.project == project,
        TestRun.timestamp >= cutoff,
        TestCase.status.in_(['failed', 'error'])
    ).all()

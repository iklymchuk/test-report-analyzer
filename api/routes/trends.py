...
async def detect_trend_anomalies(
    ...
    std_threshold: float = Query(2.0, ge=0.5, le=5.0, description="Z-score threshold for anomalies"),
    db: Session = Depends(get_db)
):
    """
...
async def compare_periods(
    ...
    period2_days: int = Query(30, ge=1, le=365, description="Comparison period in days"),
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "daily_trends": daily_trends[-14:] if len(daily_trends) > 14 else daily_trends,  # Last 2 weeks
    ...
    **Returns:**
    ...
    try:
        ...
        if not trends:
            raise HTTPException(
                status_code=404,
                detail=f"No data found for project '{project}'"
            )
        ...
        if total_runs == 0:
            ...
            avg_failure_rate = sum(t.get('failure_rate', 0) for t in trends) / len(trends)
            ...
            if len(failure_rates) > 1:
                variance = sum((r - avg_failure_rate) ** 2 for r in failure_rates) / len(failure_rates)
        ...
        return {
            ...
            "grade": grade,
            "breakdown": {
                "success_rate": round(1 - (avg_failure_rate / 100), 3) if total_runs > 0 else 0,

...
class TestRunRepository:
    ...
    def create_test_run(self, data: Dict[str, Any]) -> TestRun:
        ...
        for case_data in test_cases_data:
            ...
            filtered_case = {
                k: v for k, v in case_data.items() if k in valid_case_fields
            }
            test_case = TestCase(test_run_id=test_run.id, **filtered_case)
    ...
    def get_by_project(
        self, project: str, days: int = 30, branch: Optional[str] = None
    ) -> List[TestRun]:
        """
        ...
        query = self.db.query(TestRun).filter(
            TestRun.project == project, TestRun.timestamp >= cutoff
        )
...
class TestCaseRepository:
    ...
    def get_by_test_run(self, test_run_id: int) -> List[TestCase]:
        ...
        """
        return self.db.query(TestCase).filter(TestCase.test_run_id == test_run_id).all()

    def get_failed_tests(self, project: str, days: int = 7) -> List[TestCase]:
        """
    ...
    def get_test_history(
        ...
        """
        query = self.db.query(TestCase).join(TestRun).filter(TestCase.name == test_name)
    ...
    def get_slow_tests(
        self, project: str, threshold_seconds: float = 5.0, min_runs: int = 3
    ) -> List[Dict[str, Any]]:
        """
...
def bulk_create_test_cases(
    db: Session, test_run_id: int, test_cases: List[Dict[str, Any]]
) -> int:
    """
    ...
    """
    objects = [
        TestCase(test_run_id=test_run_id, **case_data) for case_data in test_cases
    ]

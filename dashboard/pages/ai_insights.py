...
def fetch_comprehensive_data(api_url: str, project: str, days: int) -> dict:
    ...
    try:
        # Health score
        response = requests.get(f"{api_url}/api/v1/health-score/{project}", 
                              params={"lookback_days": days}, timeout=5)
        if response.status_code == 200:
            data['health'] = response.json()
        
        # Flaky tests
        response = requests.get(f"{api_url}/api/v1/tests/flaky",
                              params={"project": project, "lookback_days": days}, timeout=5)
        if response.status_code == 200:
            data['flaky'] = response.json()
        
        # Slow tests
        response = requests.get(f"{api_url}/api/v1/tests/slow",
                              params={"project": project, "threshold_seconds": 5.0}, timeout=5)
        if response.status_code == 200:
            data['slow'] = response.json()
        
        # Failure clusters
        response = requests.get(f"{api_url}/api/v1/failures/clusters",
                              params={"project": project, "lookback_days": days}, timeout=5)
        if response.status_code == 200:
...
def generate_recommendations(data: dict, analysis_type: str) -> str:
    ...
    if flaky_count > 0:
        recommendations.append(f"1. **Address Flaky Tests**: Prioritize fixing {min(flaky_count, 5)} most critical flaky tests")
    
    if slow_count > 0:
        recommendations.append(f"2. **Optimize Performance**: Target top {min(slow_count, 10)} slow tests for optimization")
    
    if cluster_count > 0:
        recommendations.append(f"3. **Fix Systemic Issues**: Investigate {cluster_count} failure pattern(s) for root causes")
    
    if not recommendations:
        recommendations.append("1. **Maintain Excellence**: Continue current testing practices")
        recommendations.append("2. **Add Coverage**: Consider expanding test coverage for edge cases")
...
def generate_priority_actions(data: dict) -> str:
    ...
    if health_score < 50:
        ...
        actions.append("⚠️ **HIGH**: Allocate 20% of sprint capacity to test improvements")
    else:

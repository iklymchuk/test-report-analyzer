...
def render(api_url: str, project: str, days: int):
    ...
    try:
        runs_response = requests.get(
            f"{api_url}/api/v1/runs", params={"project": project, "limit": 1}, timeout=5
        )
    ...
    try:
        trends_response = requests.get(
            f"{api_url}/api/v1/trends/{project}", params={"days": days}, timeout=5
        )
        ...
        if runs_response.status_code == 200:
            ...
            if runs:
                ...
                if "timestamp" in df_runs.columns:
                    df_runs["timestamp"] = pd.to_datetime(
                        df_runs["timestamp"], format="ISO8601"
                ...
                available_columns = [
                    col for col in display_columns.keys() if col in df_runs.columns
                ]

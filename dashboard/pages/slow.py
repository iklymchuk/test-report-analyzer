...
def render(api_url: str, project: str, days: int):
    ...
    try:
        response = requests.get(
            ...
            params={"project": project, "threshold_seconds": threshold, "limit": limit},
            timeout=10,
        )
        ...
        if response.status_code == 200:
            ...
            if slow_tests:
                st.warning(
                    f"Found {len(slow_tests)} slow test(s) exceeding {threshold}s threshold"
                )
                ...
                df_display = df.copy()
                df_display["category"] = df_display["avg_duration"].apply(
                    categorize_speed
                )

                # Format durations
                df_display["avg_duration"] = df_display["avg_duration"].apply(
                    lambda x: f"{x:.2f}s"
                )
                df_display["max_duration"] = df_display["max_duration"].apply(
                    lambda x: f"{x:.2f}s"
                )

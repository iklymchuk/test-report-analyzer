...
def render(api_url: str, project: str, days: int):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            flaky_tests = [t for t in flaky_tests if t.get('flakiness_score', 0) >= min_flakiness]
            ...
            if flaky_tests:
                ...
                # Format columns
                df_display['flakiness_score'] = df_display['flakiness_score'].apply(lambda x: f"{x:.3f}")
                ...
                for idx, row in df.head(5).iterrows():
                    ...
                    with st.expander(f"**{test_name}** (Score: {score:.3f})"):
                        ...
                        with col_b:
                            ...
                            pass_rate = ((row.get('total_runs', 1) - row.get('failures', 0)) / 
                                       row.get('total_runs', 1)) * 100

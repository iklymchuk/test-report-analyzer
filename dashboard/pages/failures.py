        ...
        if response.status_code == 200:
            ...
            if clusters:
                ...
                total_affected_tests = len(set(
                    test 
                    for c in clusters 
                    for test in c.get('affected_tests', [])
                ))
                ...
                for i, cluster in enumerate(clusters, 1):
                    ...
                    sample_error = cluster.get('sample_error', 'No error message available')
                    ...
                    with st.expander(
                        ...
                        if len(affected_tests) > display_limit:
                            st.write(f"*... and {len(affected_tests) - display_limit} more*")
                        ...
                        if 'timeout' in pattern_lower or 'timed out' in pattern_lower:
                            ...
                            """)
                        elif 'connection' in pattern_lower or 'connect' in pattern_lower:
                            st.info("""
                            ...
                            """)
                        elif 'assertion' in pattern_lower or 'expected' in pattern_lower:
                            st.info("""
                ...
                try:
                    ...
                    if anomalies_response.status_code == 200:
                        ...
                        if anomalies:
                            st.warning(f"Detected {len(anomalies)} anomaly(ies) in test behavior")
                            ...
                            for anomaly in anomalies[:5]:  # Show top 5
                                ...
                                with st.expander(f"⚠️ {test_name} - {anomaly_type}"):
                                    ...
                                    if 'current_value' in anomaly:
                                        st.write(f"**Current Value:** {anomaly['current_value']}")
                ...
                st.balloons()
                st.info("This is great! Your tests are failing for diverse reasons, suggesting good test coverage without systemic issues.")
    ...
    try:
        ...
        if runs_response.status_code == 200:
            ...
            if failed_test_names:
                ...
                st.write(f"**Most Frequently Failing Tests (Last {len(recent_runs)} runs):**")
                
                df_failures = pd.DataFrame(
                    failure_counts.most_common(15),
                    columns=['Test', 'Failure Count']
                )

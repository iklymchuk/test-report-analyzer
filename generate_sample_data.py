...
def generate_sample_data(num_runs: int = 30):
    ...
    for i in range(num_runs):
        ...
        data["test_run"]["branch"] = random.choice(
            ["main", "main", "main", "feature/new-api", "feature/refactor"]
        )
            ...
            if test_case["name"] in flaky_tests:
                ...
                if random.random() < 0.3:  # 30% chance to flip status
                    if test_case["status"] == "passed":
                        ...
                        test_case["error_type"] = random.choice(
                            ["TimeoutError", "ConnectionError", "AssertionError"]
                        )
                        test_case["error_message"] = (
                            f"{test_case['error_type']}: Flaky failure at run {i+1}"
                        )
        ...
        if 15 <= i <= 18:
            for test_case in data["test_cases"]:
                if random.random() < 0.3 and test_case["status"] == "passed":
                    ...
                    test_case["error_message"] = (
                        "DeploymentError: Database migration failed during deployment"
                    )
        ...
        test_run["duration_seconds"] = sum(
            tc.get("duration_seconds", 0) or 0 for tc in data["test_cases"]
        )
        ...
        try:
            ...
            print(
                f"  ✓ Run {i+1}: {test_run['passed']}/{test_run['total_tests']} passed, "
